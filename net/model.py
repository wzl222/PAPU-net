...
class WaterTokenEncoder(nn.Module):
    def __init__(self, token_dim=128, stat_dim=10, hidden_dim=64, bias=False):
        ...
        self.semantic_encoder = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(hidden_dim, token_dim, kernel_size=3, stride=2, padding=1, bias=bias),
            nn.GELU(),
    ...
    def _stats(self, x):
        ...
        luminance_mean = luminance.mean(dim=(-2, -1))
        luminance_std = luminance.std(dim=(-2, -1), unbiased=False)
...
class WaterAwareModulation(nn.Module):
    def __init__(self, dim, token_dim=128):
        super(WaterAwareModulation, self).__init__()
        self.to_gamma_beta = nn.Sequential(
            nn.Linear(token_dim, dim * 2),
            nn.GELU(),
    ...
    def forward(self, x, water_token):
        gamma, beta = self.to_gamma_beta(water_token).chunk(2, dim=1)
...
class ColorCorrectionHead(nn.Module):
    def __init__(self, hidden_dim=24, max_residual=0.1, bias=False):
        super(ColorCorrectionHead, self).__init__()
        self.max_residual = max_residual
        self.body = nn.Sequential(
            nn.Conv2d(6, hidden_dim, kernel_size=3, stride=1, padding=1, bias=bias),
            nn.GELU(),
            nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, stride=1, padding=1, bias=bias),
            nn.GELU(),
            nn.Conv2d(hidden_dim, 3, kernel_size=1, stride=1, padding=0, bias=bias),
        )
        nn.init.zeros_(self.body[-1].weight)
        if self.body[-1].bias is not None:
            nn.init.zeros_(self.body[-1].bias)

    def forward(self, restored, inp_img):
        residual = torch.tanh(self.body(torch.cat([restored, inp_img], dim=1)))
...
class PromptIR(nn.Module):
    def __init__(self, 
        ...
        if self.water_aware:
            ...
            self.water_mod_latent = WaterAwareModulation(int(dim*2**3), water_token_dim)
